        ...
        <form method="post" action="{{ route('list-store') }}">
            ...
            <div class="create-list-form-header">
                ...
                <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="listName" name="listName" value="{{ old('listName') }}"
                        placeholder="Enter the List's Name" required />
                </div>
                <div class="mb-3">
                    <label for="listNotes" class="form-label">List Notes</label>
                    <textarea class="form-control" id="listNotes" name="listNotes" aria-describedby="listHelp"
                        placeholder="Things like trip location, milage, weather and /or group size.">{{ old('listNotes') }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="sortBy" class="form-label">Sort By</label>
                    <select class="form-control" id="sortBy" name="sortBy">
                        @foreach ($sortOptions as $option)
                            <option value="{{ $option->value }}" @if (old('sortBy') === $option->value) selected @endif>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="listClass" name="listClass" required>
                        <option value="" @if (old('listClass') === '') selected @endif>Select Style of Hiking
                        </option>
                        @foreach ($listClasses as $class)
                            <option value="{{ $class->type }}" @if (old('listClass') === $class->type) selected @endif>
