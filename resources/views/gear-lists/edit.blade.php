...
@section('content')
    ...
    <div class="btn-header-container">
        <div class="row">
            <div class="col-md-4">
                <a href="/gear-lists" class="btn btn-primary btn-sm mt-2 py-2 px-3" style="float:left;margin-left:12%">Gear
                    Lists</a>
    ...
    <div class="list-header-container">
        ...
        <form method="post" action="{{ route('list-edit') }}">
            ...
            <div class="create-list-form-header mt-4">
                @csrf
                <div class="mb-3">
                    <label for="listName" class="form-label">Name</label>
                    <input type="text" class="form-control" id="listName" name="name"
                        value="{{ $gearList->name ?? (old('name') ?? '') }}" placeholder="Enter the List's Name" required />
                </div>
                <div class="mb-3">
                    <label for="listNotes" class="form-label">List Notes</label>
                    <textarea class="form-control" id="listNotes" name="notes" aria-describedby="listHelp"
                        placeholder="Things like trip location, milage, weather and /or group size.">{{ $gearList->notes ?? (old('notes') ?? '') }}</textarea>
                </div>
                <div class="mb-3">
                    <label for="sortBy" class="form-label">Sort By</label>
                    <select class="form-control" id="sortBy" name="sort">
                        @foreach ($sortOptions as $option)
                            <option value="{{ $option->value }}" @if ($gearList->sort === $option->value) selected @endif>
                ...
                <div class="mb-3">
                    ...
                    <select class="form-control" id="listClass" name="list_class" required>
                        <option value="" @if ($gearList->list_class === '') selected @endif>Select Style of Hiking
                        </option>
                        @foreach ($listClasses as $class)
                            <option value="{{ $class->type }}" @if ($gearList->list_class === $class->type) selected @endif>
                                {{ $class->display . ' - ' . $class->description }}</option>
                        @endforeach
                    </select>
